...
if [ $stage -le 5 ]; then
  # make features
  local/extract_wav2vec_features.sh data/train exp/wav2vec/train $wav2vec 
fi

if [ $stage -le 6 ]; then
  local/extract_wav2vec_features.sh data/test exp/wav2vec/test $wav2vec exp/wav2vec/train/h5/remove_dims 
fi
