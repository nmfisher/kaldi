...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc_pitch_online.sh --cmd "$train_cmd" --nj 1 data/${train_set}_sp || exit 1;
...
if [ $stage -le 3 ]; then
  ...
  utils/subset_data_dir.sh data/${train_set}_sp_hires 120 \
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 1 \
    ...
    ${temp_data_root}/${train_set}_sp_hires_subset 128 \
