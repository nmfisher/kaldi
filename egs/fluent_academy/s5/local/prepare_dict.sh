...
cat $train_dir/text $dev_dir/text | awk '{for (i = 2; i <= NF; i++) print $i}' |\
  perl -ape 's/ /\n/g;' | sort -u | grep -v '\[FIL\]' |\
  grep -v '\[SPK\]' > $dict_dir/words.txt || { echo "Couldn't extract vocab"; exit 1; };

# split into English and Chinese
cat $dict_dir/words.txt | grep '[a-zA-Z]' > $dict_dir/lexicon-en/words-en.txt;
cat $dict_dir/words.txt | grep -v '[a-zA-Z]' > $dict_dir/lexicon-ch/words-ch.txt || { echo "Couldn't create Chinese lexicon"; exit 1; };
...
perl $dict_dir/cmudict/scripts/make_baseform.pl \
  ...
  sed -e 's:^\([^\s(]\+\)([0-9]\+)\(\s\+\)\(.*\):\1\2\3:' > $dict_dir/cmudict/cmudict-plain.txt || { echo "Failed to strip markers from cmudict"; exit 1; };
